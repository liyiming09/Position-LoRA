...
def _find_modules_v3(
    ...
    v3: v2 + full_name of mudules in unet, such as "mid_block.attentions.0.transformer_blocks.0.ff.net.0"
    """
    ...
    if ancestor_class is not None:
        ancestors = (
            (pa_name, module)
            for pa_name, module in model.named_modules()
            if module.__class__.__name__ in ancestor_class
        )
    else:
        # this, incase you want to naively iterate over all modules.
    ...
    # For each target find every linear_class module that isn't a child of a LoraInjectedLinear
    for pa_name, ancestor in ancestors:
        for fullname, module in ancestor.named_modules():
            if any([isinstance(module, _class) for _class in search_class]):
                # Find the direct parent if this is a descendant, not a child, of target
                *path, name = fullname.split(".")
                ...
                if exclude_children_of and any(
                    ...
                    continue
                # Otherwise, yield it
                yield parent, pa_name + '.' + name, module
...
def extract_lora_names(model, target_replace_module=None):
    if target_replace_module is None:
        target_replace_module = DEFAULT_TARGET_REPLACE
    loras = []

    for _m, _n, _child_module in _find_modules_v3(
            model,
            target_replace_module,
            search_class=[LoraInjectedLinear, LoraInjectedConv2d],
    ):
        loras.append(_n+'.lora_up')
        loras.append(_n+'.lora_down')
...
def save_lora_layername(
    ...
    for _name in extract_lora_names(
            model, target_replace_module=target_replace_module
    ):
        names.append(_name)
